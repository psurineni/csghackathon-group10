...
$db= "select * from meetings where name='".$_SESSION['name']."'";
...
<div id = "container" width= "30%">
	...
	<div class="centerthis">
		<div id="chatlog">
			...
			<span class="botTitle">chatbot: </span>Hey <?php 
			$db1= "select * from meetings where name='".$_SESSION['name']."'";

	$query1= mysqli_query($connection,$db1);
	...
	//$rows1=mysqli_fetch_array($query1,MYSQLI_ASSOC);
	 ...
	 //echo date($bday);
	 //echo date("Y/m/d");
	 if (date($bday) ==date("Y-m-d"))
	 {
		echo "Chatbot is wishing you a very hapy birthday!"; 
	 }
	 else 
	 {
		 echo "Its your Friends birthday today let's surprise him";
	 }
	 //echo '<br/>';
	...
	<div id="urlwarning"><?php $display ?></div>
	<form action="multibot_gui_with_chatlog.php" method="GET"	onsubmit="return allLetter(name)">
		<div><label >Please Enter a url you need to save:</label>
		<input type="text" name="url" placeholder="url" autocomplete="off" required> &emsp; 
		<br><br><button type="submit">Generate suggestions</button></div>
		</form>
		<?php $numProducts = rand(3,6);
	if(isset($_GET['url'])){
		$_SESSION['url']= $url;
	$db1R= "select * from list ORDER BY RAND() LIMIT $numProducts";	
	$query1R= mysqli_query($connection,$db1R);	
	while($rows1R = mysqli_fetch_array($query1R,MYSQLI_ASSOC))
	{
		echo $rows1R['tags']."</br>";
	}
	}
	?>
		<form action="multibot_gui_with_chatlog.php" method="GET"	onsubmit="return allLetter(name)">
		<div><label >Please Enter a tag to pin this url:</label>
		<input type="text" name="tag" placeholder="tag" autocomplete="off" required>
				
								<br><br><button type="submit">Save</button></div>
	<?php 
	...
	if(isset($_GET['tag']))
	{
	$db1R= "INSERT INTO `tags`(`tag`, `url`) VALUES ('".$_GET['tag']."','".$_SESSION['url']."')";
	$query1R= mysqli_query($connection,$db1R);		
	}
	?>
	</form>
